...
function meta(a) {
  return `<div style="font-family: Geist, sans-serif; font-weight: 400; font-size: 14px; color: rgb(111, 106, 99); display: flex; gap: 16px; margin-top: 16px; flex-wrap: wrap;"><span>${escapeHtml(a.author)}</span><span>${fmtDate(a.date)}</span><span>${readMin(a)} min read</span></div>`;
}
...
function featuredCard(a) {
  ...
  <div style="display: flex; flex-direction: column; justify-content: center;">
    <div style="font-family: Geist, sans-serif; font-weight: 300; font-size: 13px; letter-spacing: .12px; text-transform: uppercase; color: rgb(107,76,205);">${escapeHtml(a.category)}</div>
    <div class="ptitle"${nt} style="font-family: Geist, sans-serif; font-weight: 800; font-size: 30px; line-height: 1.16; color: rgb(23,24,28); margin-top: 10px;">${escapeHtml(a.title)}</div>
    <p${nt} style="font-family: Geist, sans-serif; font-weight: 400; font-size: 16px; line-height: 27px; color: rgb(38,38,38); margin: 12px 0 0;">${escapeHtml(a.excerpt)}</p>
    ${meta(a)}
  </div></a>`;
}
...
function gridCard(a, h = 318) {
  ...
  <div>
    <div style="font-family: Geist, sans-serif; font-weight: 300; font-size: 12px; letter-spacing: .12px; text-transform: uppercase; color: rgb(107,76,205);">${escapeHtml(a.category)}</div>
    <div class="ptitle"${nt} style="font-family: Geist, sans-serif; font-weight: 700; font-size: 20px; line-height: 1.18; color: rgb(23,24,28); margin-top: 10px;">${escapeHtml(a.title)}</div>
    <p${nt} style="font-family: Geist, sans-serif; font-weight: 400; font-size: 15px; line-height: 26px; color: rgb(38,38,38); margin: 10px 0 0;">${escapeHtml(a.excerpt)}</p>
    ${meta(a)}
  </div></a>`;
}
...
function relatedCard(a) {
  ...
  return `<a class="post" href="/journal/${escapeHtml(a.slug)}" style="display:flex;flex-direction:column;gap:12px;">
    ...
    <div class="ptitle" style="font-family:Geist,sans-serif;font-weight:700;font-size:18px;line-height:1.2;color:rgb(23,24,28);">${escapeHtml(a.title)}</div>
  </a>`;
}
